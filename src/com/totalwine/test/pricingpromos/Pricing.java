...
public class Pricing extends Browser {
	...
	public void PricingTest () throws InterruptedException, BiffException, IOException, AWTException {
	    ...
	    sAssert.assertTrue(plpPrice.equals(pdpPrice),"PLP price is not the same as PDP price");
//	    Assert.assertTrue(pdpPrice.equals(cartUnitPrice),"PDP price is not the same as unit price in cart");
	    sAssert.assertTrue(cartUnitPrice.equals(cartTotalPrice),"Unit price in cart is not the same as total price in cart for the single item");
	    sAssert.assertTrue(elpPrice.equals(edpPrice),"ELP price is not the same as EDP price");
	}
}